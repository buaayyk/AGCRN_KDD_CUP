...
def forecast_one(experiment, model_folder):
    ...
    path_to_model = os.path.join(args["checkpoints"], model_folder, 'model_{}'.format(str(args["group_id"])))
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y) in enumerate(test_loader):
            ...
            predictions.append(np.array(sample.cpu()))
            true_lst.append(np.array(true.cpu()))
    predictions = np.array(predictions)
    true_lst = np.array(true_lst)
    predictions = predictions.reshape(-1, predictions.shape[-3], predictions.shape[-2], predictions.shape[-1])
    true_lst = true_lst.reshape(-1, true_lst.shape[-3], true_lst.shape[-2], true_lst.shape[-1])
...
def forecast(settings):
    ...
    test_size = settings['day_len'] * settings['test_size']
    for j in range(settings["groups"]):
        pred, gt, raw_data = results[j]
        for i in range(pred.shape[-2]):
            preds.append(pred[:, :, i, :])
            gts.append(gt[:, :, i, :])
            raw_data_ls.append(raw_data[i * test_size:(i + 1) * test_size])
